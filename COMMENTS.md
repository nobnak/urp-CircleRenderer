# Implementation notes — jp.nobnak.circle

Design notes, shader conventions, and file map for maintainers. The package lives under `Packages/jp.nobnak.circle/`.
...
| Batch cap            | Accumulation / array sizing: `kMaxInstancesPerDraw == 1023`. **Each** `Graphics.DrawMeshInstanced` call uses at most `kMaxGpuInstancesPerDraw == 511` instances so `StructuredBuffer` slices match `SV_InstanceID` (default matrix instancing uses objectToWorld + worldToObject per instance; Unity documents a max of **511** per draw for that case). |
...
**LateUpdate**  
Only `CircleInstancedRenderer` and `RingInstancedRenderer` should call `DrawAccumulatedFrame()` from `void LateUpdate() => DrawAccumulatedFrame();`. Putting this only on the base can fail to run in some setups.
