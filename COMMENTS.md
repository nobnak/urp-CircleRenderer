...
| Batch cap            | Accumulation / array sizing: `kMaxInstancesPerDraw == 1023`. **Each** `Graphics.DrawMeshInstanced` call splits at `kMaxGpuInstancesPerDraw == 512` so `StructuredBuffer` slices match `SV_InstanceID` when Unity splits matrix instancing (default objectToWorld + worldToObject CB ~64KB). |
